...
router.get("/:id", async (req, res) => {
  ...
  const filtered = similarProducts.filter(p => p._id.toString() != product._id.toString());
...
router.post('/search/item', async (req, res) => {
  ...
  const matches = s.matchArray(products, {title: req.body.search, _text: true});

  return res.render('products/search', {matches, isLogged: req.session.isLogged});
})
