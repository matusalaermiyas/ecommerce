...
router.post('/express', async (req, res) => {
    ...
    return res.json({result: false, available: product.item_in_stock, label: product.title})
});
...
router.post('/cart', async (req, res) => {
    ...
    products.forEach(p => {
        if(!mongoose.isValidObjectId(p.ItemId)) return res.status(400).json({"message": "Invalid product id"});
    })
    ...
    for(var i = 0; i < products.length; i++)
    {
        ...
        {
            errorMessages.push({available: productInStock.item_in_stock, label: productInStock.title});
        }
